# CLAUDE.md — xiaozhi-esp32 定制分支（Fridge on bread-compact-wifi-epaperx）

> 本文件供 AI 编码助手（Claude Code 等）快速上手。**不要通读整个仓库**——70+ 板型是上游通用代码，本分支定制点只有一个：下方「定制范围」。任务触及哪块再读对应 `docs/agent/*.md`。

## 项目身份
- 上游：[78/xiaozhi-esp32](https://github.com/78/xiaozhi-esp32)，ESP32 语音 AI 聊天机器人（MCP 协议控物）。
- 本分支定制点：在 `bread-compact-wifi-epaperx` 板型上加了完整「冰箱管理」功能（Fridge 模块：NVS 持久化 + 11 个 MCP 工具 + 墨水屏多页面）。
- **当前 active board**：`bread-compact-wifi-epaperx`（见 `sdkconfig` 的 `CONFIG_BOARD_TYPE_BREAD_COMPACT_WIFI_EPAPERX=y`）。

## 一条构建命令
```bash
idf.py build            # 编译（需 ESP-IDF 5.4+，已设置 ESP-IDF 环境变量）
idf.py -p <PORT> flash  # 烧录
```
切换板型见 `docs/agent/build-and-boards.md`。详细构建/分区/assets 见同上文档。
...
| `main/application.{h,cc}` | 应用主循环；MCP 初始化入口（`AddCommonTools`/`AddUserOnlyTools`） |
| `main/boards/common/board.h` | `Board` 基类（含 `GetEpaperDisplay()`） |
| `main/CMakeLists.txt` | 源文件列表 + `BOARD_TYPE` 选择机制（超大，~815 行） |

## 代码风格
- Google C++ 风格（上游约定，README 已声明）。
- 注释用中文（与 Fridge 模块既有风格一致）。
- ESP-IDF 日志用 `ESP_LOGI/W/E/D/V` + `TAG`。
...
- `components/*`（除 `components/GxEPD2/`，后者含墨水屏局部刷新 LUT 定制，见 `.gitignore` 白名单）
- 70+ 其他板型目录（非本分支目标）
